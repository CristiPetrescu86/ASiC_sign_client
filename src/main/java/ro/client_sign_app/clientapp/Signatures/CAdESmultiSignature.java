...
public class CAdESmultiSignature {

    private CommonCertificateVerifier commonCertificateVerifier;
    private ArrayList<CAdESService> services;
    private ArrayList<CAdESSignatureParameters> parametersList;
    private ArrayList<DSSDocument> documentsToBeSigned;

    public CAdESmultiSignature(Integer instances){
        parametersList = new ArrayList<>();
        services = new ArrayList<>();
        documentsToBeSigned = new ArrayList<>();

        for (int i = 0; i < instances; i++){
            parametersList.add(new CAdESSignatureParameters());
            commonCertificateVerifier = new CommonCertificateVerifier();
            services.add(new CAdESService(commonCertificateVerifier));
        }
    }

    public ArrayList<ToBeSigned> doSignatureCSC(ArrayList<String> docPath, SignatureLevel signatureLevel, DigestAlgorithm digestAlgorithm, Cred_info_resp keyInfo){
        try {
            ...
            for (int i = 0; i < docPath.size(); i++){
                ...
                parametersList.get(i).setSignaturePackaging(SignaturePackaging.ENVELOPING);
    ...
    public ArrayList<DSSDocument> integrateSignatureCSC(List<String> signedHash, SignatureAlgorithm SignAlgo)
    {
